...
class JmEnquiryScreen extends StatefulWidget {
  const JmEnquiryScreen({super.key});

  @override
  State<JmEnquiryScreen> createState() => _JmEnquiresScreenState();
}

class _JmEnquiresScreenState extends State<JmEnquiryScreen> {
  final JmEnquiryController jmEnquiryController =
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      jmEnquiryController.refreshEnquiries();
    });
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: jmEnquiryController.refreshEnquiries,
          ),
        ],
      ),
      body: SafeArea(
        child: Padding(
          ...
          child: Obx(() {
            if (jmEnquiryController.isLoading.value &&
                jmEnquiryController.enquiryItems.isEmpty) {
              return Center(child: CircularProgressIndicator());
            }

            if (jmEnquiryController.hasError.value &&
                jmEnquiryController.enquiryItems.isEmpty) {
              return _errorWidget();
            }

            if (jmEnquiryController.enquiryItems.isEmpty) {
              return _noEquires();
            }

            return RefreshIndicator(
              onRefresh: jmEnquiryController.refreshEnquiries,
  ...
  Widget _buildEnquiryList() {
    final groupedEnquiries = jmEnquiryController.getEnquiriesByDate();
    ...
    return Column(
      ...
      children: [
        ...
        if (jmEnquiryController.isLoading.value &&
            jmEnquiryController.enquiryItems.isNotEmpty)
          Center(child: CircularProgressIndicator()),
        if (jmEnquiryController.hasMore.value)
          Center(
            child: TextButton(
              onPressed: jmEnquiryController.loadMoreEnquiries,
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          SizedBox(height: 10),
          Text(
            jmEnquiryController.errorMessage,
