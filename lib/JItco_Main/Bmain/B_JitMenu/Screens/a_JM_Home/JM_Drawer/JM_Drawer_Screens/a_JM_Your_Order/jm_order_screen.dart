...
import 'package:jitco_app/JItco_Main/Bmain/B_JitMenu/Screens/a_JM_Home/JM_Drawer/JM_Drawer_Screens/a_JM_Your_Order/jm_order_detail_screen.dart';
// import 'package:jitco_app/screens/Bmain/Jitco_Supply_Home/A_models/order_model.dart';
...
class JmOrderScreen extends StatefulWidget {
  const JmOrderScreen({super.key});

  @override
  State<JmOrderScreen> createState() => _JmOrdersScreenState();
}

class _JmOrdersScreenState extends State<JmOrderScreen> {
  final JmOrderController orderController = Get.find<JmOrderController>();
  ...
  // List<OrdersResponse> get filteredOrders {
  //   if (selectedStatus == OrderStatus.all) {
  //     return orderController.orders.toList();
  //   }

  //   return orderController.orders.where((order) {
  //     final orderStatus = order.orders[index].orderStatus;
  ...
  List<OrderDetails> get filteredOrders {
    final allOrders = orderController.orders.toList();

    if (selectedStatus == OrderStatus.all) return allOrders;

    return allOrders.where((order) {
      final orderStatus = order.orderStatus?.toLowerCase() ?? '';
      final selected = selectedStatus!.name.toLowerCase();

      if (selected == 'pending' && orderStatus == 'processing') return true;
  ...
  void _addToCart(
    ...
    jmCartController.addToCart(cartItem);
              ...
              ...order.items!
                  ...
                  .map(
                    (item) => ListTile(
                      ...
                      title: item.product!.productName!.text.size(14).make(),
            ...
            Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    "Total Items: ${order.items?.length}".text
                        .size(12)
                        .color(Colors.grey[600])
                        .make(),
                ...
                Column(
                  crossAxisAlignment: CrossAxisAlignment.end,
                  children: [
                    '₹${order.finalAmount!}'.text
                        .size(18)
                        .fontWeight(FontWeight.w600)
                        .color(Colors.orange.shade700)
                        .make(),
                    "Total: ₹${order.totalAmount!.toStringAsFixed(2)} + Tax: ₹${(order.gst)?.toStringAsFixed(2)}"
                        .text
                        .size(10)
                        .color(Colors.grey[600])
                        .make(),
                ...
                Expanded(
                  child: SizedBox(
                    ...
                    child: OutlinedButton(
                      onPressed: () {
                        Get.to(() => JmOrderDetailScreen(orderId: order.id!));
                      },
                      style: OutlinedButton.styleFrom(
                        side: BorderSide(color: Colors.orange.shade700),
                      ),
