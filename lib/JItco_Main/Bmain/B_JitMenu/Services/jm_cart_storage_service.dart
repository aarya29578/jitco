...
import 'package:jitco_app/JItco_Main/Bmain/B_JitMenu/Enums/cart_enum.dart';
...
class JmCartStorageService {
  final _storage = GetStorage();
  String _cartKey = 'jit_menu_items';

  // void cartType(CartType cartTypes) {
  //   if (cartTypes == CartType.food) {
  //     _cartKey = 'supply_cart_items';
  //   } else if (cartTypes == CartType.jmMenu) {
  //     _cartKey = 'jit_menu_items';
  //   } else {
  //     print("no stored for other carts");
  //   }
  // }

  // Save cart items to storage
  void saveCartItems(List<JmCartItem> cartItems) {
  ...
  List<JmCartItem> getCartItems() {
    final items = _storage.read<List>(_cartKey);
    if (items != null) {
      return items
          .map((itemMap) => JmCartItem.fromMap(itemMap.cast<String, dynamic>()))
          .toList();
    }
