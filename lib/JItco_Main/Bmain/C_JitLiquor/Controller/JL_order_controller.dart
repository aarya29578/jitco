...
class JlOrderController extends GetxController {
  final JlApiService _apiService = Get.find<JlApiService>();

  RxList<JlOrder> orders = <JlOrder>[].obs;
  ...
  Future<void> loadOrders({int page = 1, String status = 'All'}) async {
    try {
      ...
      final response = await _apiService.jlGetOrder(
        page: page,
        limit: 20,
        status: status,
      );
      ...
      if (response['success'] == true) {
        ...
        final newOrders = ordersData
            .map((json) => JlOrder.fromJson(json as Map<String, dynamic>))
            .toList();
  ...
  // Get order by ID
  JlOrder? getOrderById(String id) {
