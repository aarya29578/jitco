...
// class JlCartcontroller extends GetxController {
//   var cartItems = <JlCartitem>[].obs;
...
//   void addToCart(JlCartitem newItem) {
...
//   void updateCartItem(String itemId, JlCartitem updatedItem) {
...
class CartStorageService {
  ...
  final String _cartKey = 'cart_liquor';

  // Save cart items to storage
  void saveCartItems(List<JlCartitem> cartItems) {
  ...
  List<JlCartitem> getCartItems() {
    final items = _storage.read<List>(_cartKey);
    if (items != null) {
      return items
          .map((itemMap) => JlCartitem.fromMap(itemMap.cast<String, dynamic>()))
          .toList();
    }
    return [];
  }
...
class JlCartcontroller extends GetxController {
  ...
  void removeFromCart(String itemId, String variant) {
    cartItems.removeWhere((item) => item.id == itemId && item.uom == variant);
    _saveCartToStorage();
