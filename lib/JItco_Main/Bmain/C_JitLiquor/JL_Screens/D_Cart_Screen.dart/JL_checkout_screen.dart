...
class JlCheckoutScreen extends StatefulWidget {
  const JlCheckoutScreen({super.key});

  @override
  State<JlCheckoutScreen> createState() => _JlCheckoutScreenState();
}

class _JlCheckoutScreenState extends State<JlCheckoutScreen> {
  ...
  final JlApiService jlApiService = Get.find<JlApiService>();
  final JlPriceCalculationServices priceService =
      Get.find<JlPriceCalculationServices>();
  ...
  void initState() {
    ...
    for (var item in jlCartController.cartItems) {
      if (item.originalPrice == null) {
        ...
        final index = jlCartController.cartItems.indexOf(item);
        jlCartController.cartItems[index] = item.copyWith(
  ...
  void _clearSavedAddress() {
    try {
      ...
      if (jlCartController.currentWarehouseId.value.isNotEmpty) {
        jlCartController.currentWarehouseId.value = '';
      }
    } catch (e) {
  ...
  double? _calculateItemPrice(JlCartitem item) {
    ...
    final productData = jlCartController.productDetailsCache[item.id];
  ...
  String _getItemGST(JlCartitem item) {
    final productData = jlCartController.productDetailsCache[item.id];
    if (productData == null) {
  ...
  double get _calculatedTotalPrice {
    ...
    for (final item in jlCartController.cartItems) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
              ...
              DeliveryAddressSection(
                ...
                onCitySelected: (cityName) {
                  ...
                  if (cityName.isNotEmpty &&
                      jlCartController.cartItems.isNotEmpty) {
                    jlCartController.checkPriceUpdate(cityName);
                    setState(() {});
                  }
                },
      ...
      bottomNavigationBar: Obx(
        () => jlCartController.cartItems.isEmpty
  ...
  double getItemDisplayPrice(JlCartitem item) {
    ...
    if (isDeliveryAddressSelected.value &&
        jlCartController.productDetailsCache.containsKey(item.id)) {
      final productData = jlCartController.productDetailsCache[item.id]!;
          ...
          children: [
            Row(
              children: [
                ...
                Obx(
                  () => Text(
                    '${jlCartController.totalItems} items',
            ...
            Obx(
              () => ListView.builder(
                ...
                itemBuilder: (context, index) {
                  late int selectedUomIndex = jlCartController.selectedUomIndex;
                  print('1: $selectedUomIndex');
                  final item = jlCartController.cartItems[index];
                  final productData =
                      jlCartController.productDetailsCache[item.id];
                  ...
                  final calculatedGST = priceService.getItemGST(
                    productData: jlCartController.productDetailsCache[item.id],
                    item: item,
                  );
                  final gstPercentage =
      ...
      child: Padding(
        ...
        child: Column(
          children: [
            ...
            Column(
              children: [
                _buildPriceRow('Items:', '${jlCartController.totalItems}'),
                _buildPriceRow(
                  'Subtotal:',
                ...
                if (jlCartController.hasPriceChanges)
                  Container(
  ...
  Future<void> _submitOrder() async {
    ...
    try {
      ...
      String warehouseId = jlCartController.currentWarehouseId.value;
      ...
      //   "items": jlCartController.cartItems.map((item) {
      ...
      final orderPayload = {
        ...
        "items": jlCartController.cartItems.map((item) {
          ...
          final double finalPrice =
              (isDeliveryAddressSelected.value && productData != null)
              ? priceService.resolveProductPrice(
                      productData: productData,
                      selectVariantUom: item.selectedVariant,
                    ) ??
          ...
          final double gstPercentage =
              double.tryParse(
                priceService.getItemGST(productData: productData, item: item),
              ) ??
              item.gstPercentage ??
              0.0;

          final double priceTotal = finalPrice * item.quantity;
      ...
      try {
        final result = await jlApiService.jlPostOrder(orderPayload);
          ...
          if (paymentUrl != null && paymentUrl.isNotEmpty) {
            ...
            Get.to(
              () => PaymentWebView(
                ...
                onPaymentSuccess: () {
                  print("clearing cart from liquor");
                  jlCartController.clearCart();
                  Get.snackbar(
                    'Payment Successful',
            ...
            return ElevatedButton(
              onPressed: authController.isLoading.value
                  ...
                  : () async {
                      ...
                      if (jlCartController.cartItems.isEmpty) {
                        // _showSnackbar(
                        //   title: 'Error',
