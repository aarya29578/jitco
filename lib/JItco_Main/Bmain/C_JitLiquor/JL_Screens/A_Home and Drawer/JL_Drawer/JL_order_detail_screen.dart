...
import 'package:jitco_app/JItco_Main/Bmain/D_JitService/JS_Screens/A_JS_Home/JS_drawer/A_JS_Order/Download_Service/generateJSOrderPDF.dart';
import 'package:velocity_x/velocity_x.dart';
import 'package:intl/intl.dart';

class JLOrderDetailScreen extends StatefulWidget {
  ...
  final String? whichSource;

  const JLOrderDetailScreen({
    super.key,
  ...
  @override
  State<JLOrderDetailScreen> createState() => _JLOrderDetailScreenState();
}

class _JLOrderDetailScreenState extends State<JLOrderDetailScreen> {
  ...
  Widget build(BuildContext context) {
    print("orderdetailpdf$_order");
    print('orderProgress777${_order.progress}');
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            onPressed: () {
              // generateJsOrderPdf(context, _order);
              _showBasicDialog(context, () {
                generateJsOrderPdf(context, _order, widget.whichSource);
              });
            },
            icon: Icon(
      ...
      child: Column(
        ...
        children: [
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        "Price: ₹${item.price!.toStringAsFixed(2)}/per".text
                            .size(12)
                            .color(Colors.grey[600])
                            .make(),
          ...
          Column(
            ...
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              if (widget.isService == true && item.slotDate != null ||
                  item.slotTime != null) ...[
                Text(
                  "Slot at: ${item.slotDate} • ${item.slotTime}",
                  style: TextStyle(color: Colors.grey[600]),
                ),
                7.heightBox,
              ],
              "Total: ₹${item.total!.toStringAsFixed(2)}".text
                  .size(15)
                  .fontWeight(FontWeight.w600)
                  .color(Colors.orange.shade700)
                  .make(),
                  ...
                  child: Column(
                    ...
                    children: [
                      "Shipping Address".text
                          ...
                          .make(),
                      4.heightBox,
                      _order.outlet?.name != null ||
                              _order.outlet?.address != null ||
                              _order.outlet?.state?.name != null ||
                              _order.outlet?.city?.name != null ||
                              _order.outlet?.country != null ||
                              _order.outlet?.pinCode != null
                          ? Text(
                              '${_order.outlet?.name ?? ''} , ${_order.outlet?.address ?? ''}, ${_order.outlet?.state?.name ?? ''}, ${_order.outlet?.city?.name ?? ''}, ${_order.outlet?.country ?? ''} - ${_order.outlet?.pinCode ?? ''}',
                            )
